      ...
      try {
        ...
        if (token && !token.startsWith("apify_api_YOUR_TOKEN")) {
          ...
          const res = await fetch(`https://api.apify.com/v2/acts/apidojo~tweet-scraper/run-sync-get-dataset-items?token=${token}`, {
            ...
            body: JSON.stringify({ twitterHandles: handles, tweetsDesired: 1, addParentTweets: false })
          });
          if (res.ok) {
...
${tweetsContext ? `OSINT feeds:\n${tweetsContext}\n` : ""}
Select ONE real-world crisis area on Earth. Identify a real-world conflict, naval blockade, missile incident, or severe supply chain bottleneck currently occurring (2025/2026).
